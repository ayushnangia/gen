...
def assign_emotions(turns: List[Dict]) -> List[Dict]:
    """
    ...
    for turn in turns:
        ...
        try:
            ...
            primary_emotion = max(emotions[0], key=lambda x: x['score'])['label']
...
def parse_arguments():
    parser = argparse.ArgumentParser(description="Generate dialogues using OpenAI API with advanced uniqueness checks, emotion assignment, and dynamic parameter tuning.")
...
def main():
    ...
    for index in tqdm(selected_indices, desc="Generating dialogues"):
        ...
        if generated_dialogue:
            ...
            # Assign emotions to each turn
            generated_turns = assign_emotions(generated_turns)
