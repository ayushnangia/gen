...
# Performance metrics class to collect metrics
class PerformanceMetrics:
    def __init__(self):
        self.total_dialogues_attempted = 0
        self.total_dialogues_generated = 0
        self.total_retries = 0
        self.request_latencies = []
        self.total_time_taken = 0.0
        self.tokens_used_per_request = []
        self.dialogue_service_distribution = defaultdict(int)
        self.dialogue_turns = []
        self.latencies_per_dialogue = []
        self.retries_per_dialogue = []
        self.cpu_usages = []
        self.memory_usages = []
    ...
    def add(self, other):
        self.total_dialogues_attempted += other.total_dialogues_attempted
        self.total_dialogues_generated += other.total_dialogues_generated
        self.total_retries += other.total_retries
        self.request_latencies.extend(other.request_latencies)
        self.total_time_taken += other.total_time_taken
        self.tokens_used_per_request.extend(other.tokens_used_per_request)
        for service, count in other.dialogue_service_distribution.items():
            self.dialogue_service_distribution[service] += count
        self.dialogue_turns.extend(other.dialogue_turns)
        self.latencies_per_dialogue.extend(other.latencies_per_dialogue)
        self.retries_per_dialogue.extend(other.retries_per_dialogue)
        self.cpu_usages.extend(other.cpu_usages)
        self.memory_usages.extend(other.memory_usages)
        self.successful_dialogues += other.successful_dialogues
        self.failed_dialogues += other.failed_dialogues
...
class DialogueGenerator:
    ...
    async def generate_dynamic_scenario(self, category: str, listservice: list, region: str) -> Tuple[str, Tuple[int, int, str]]:
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                ...
                self.metrics.tokens_used_per_request.append(tokens_used)
    ...
    async def generate_unique_dialogue(self, idx, dataset_index):
        ...
        while retry_count < max_retries:
            try:
                self.metrics.total_dialogues_attempted += 1
                ...
                if dialogue_hash in self.existing_hashes:
                    ...
                    self.metrics.failed_dialogues += 1
                    return None
                ...
                latency = time() - start_time
                self.metrics.latencies_per_dialogue.append(latency)
                ...
                self.metrics.total_dialogues_generated += 1
                self.metrics.successful_dialogues += 1
                self.metrics.dialogue_service_distribution[primary_service] += 1
                self.metrics.dialogue_turns.append(num_lines)
                self.metrics.retries_per_dialogue.append(retry_count)
    ...
    def generate_unique_dialogues_worker(args):
        ...
        result = asyncio.run(generator.generate_unique_dialogue(index, dataset_index))
        if result:
            dialogue, embedding = result
            return (dialogue, embedding, generator.metrics)
        else:
            return (None, None, generator.metrics)
    ...
    def generate_unique_dialogues(self, num_generations: int, min_turns: int, max_turns: int,
        ...
        # Filter out None results and separate dialogues, embeddings, and metrics
        valid_results = [r for r in results if r[0] is not None]
        new_dialogues = [dialogue for dialogue, _, _ in valid_results]
        new_embeddings = np.vstack([embedding for _, embedding, _ in valid_results]) if valid_results else np.array([])
        all_metrics = [metrics for _, _, metrics in results]
        ...
        self.metrics.total_time_taken = time() - start_time
    ...
    def output_metrics(self):
        """
        Outputs the collected performance metrics.
        """
        self.logger.info("Performance Metrics:")
        self.logger.info(f"Total Dialogues Attempted: {self.metrics.total_dialogues_attempted}")
        self.logger.info(f"Total Dialogues Generated: {self.metrics.total_dialogues_generated}")
        self.logger.info(f"Total Successful Dialogues: {self.metrics.successful_dialogues}")
        self.logger.info(f"Total Failed Dialogues: {self.metrics.failed_dialogues}")
        self.logger.info(f"Total Retries: {self.metrics.total_retries}")
        if self.metrics.retries_per_dialogue:
            avg_retries = sum(self.metrics.retries_per_dialogue) / len(self.metrics.retries_per_dialogue)
            self.logger.info(f"Average Retries per Dialogue: {avg_retries:.2f}")
        if self.metrics.request_latencies:
            avg_latency = sum(self.metrics.request_latencies) / len(self.metrics.request_latencies)
            self.logger.info(f"Average Request Latency: {avg_latency:.2f} seconds")
        else:
            self.logger.info("No request latencies recorded.")
        self.logger.info(f"Total Time Taken: {self.metrics.total_time_taken:.2f} seconds")
        if self.metrics.tokens_used_per_request:
            total_tokens = sum(self.metrics.tokens_used_per_request)
            avg_tokens = total_tokens / len(self.metrics.tokens_used_per_request)
            self.logger.info(f"Total Tokens Used: {total_tokens}")
            self.logger.info(f"Average Tokens per Request: {avg_tokens:.2f}")
        if self.metrics.dialogue_service_distribution:
            self.logger.info("Dialogue Service Distribution:")
            for service, count in self.metrics.dialogue_service_distribution.items():
                self.logger.info(f"  {service}: {count}")
        if self.metrics.dialogue_turns:
            avg_turns = sum(self.metrics.dialogue_turns) / len(self.metrics.dialogue_turns)
            self.logger.info(f"Average Number of Turns per Dialogue: {avg_turns:.2f}")

    def save_metrics(self, filename='metrics.json'):
        """
        Saves the collected performance metrics to a JSON file.
        """
        metrics_data = {
            'total_dialogues_attempted': self.metrics.total_dialogues_attempted,
            'total_dialogues_generated': self.metrics.total_dialogues_generated,
            'total_successful_dialogues': self.metrics.successful_dialogues,
            'total_failed_dialogues': self.metrics.failed_dialogues,
            'total_retries': self.metrics.total_retries,
            'retries_per_dialogue': self.metrics.retries_per_dialogue,
            'request_latencies': self.metrics.request_latencies,
            'total_time_taken': self.metrics.total_time_taken,
            'tokens_used_per_request': self.metrics.tokens_used_per_request,
            'dialogue_service_distribution': dict(self.metrics.dialogue_service_distribution),
            'dialogue_turns': self.metrics.dialogue_turns
        }
        try:
    ...
    def save_metrics_csv(self, filename='metrics.csv'):
        """
        Saves the collected performance metrics to a CSV file using Pandas.
        """
        import pandas as pd
        ...
        # Determine the maximum length among lists
        ...
        # Add summary metrics
        df['total_dialogues_attempted'] = self.metrics.total_dialogues_attempted
        df['total_dialogues_generated'] = self.metrics.total_dialogues_generated
        df['total_time_taken'] = self.metrics.total_time_taken
...
def main():
    ...
    if args.test_extraction:
        ...
        generator.output_metrics()
        generator.save_metrics()
        generator.save_metrics_csv()
