...
def load_data(uploaded_file):
    ...
    for dialogue in data:
        if not required_fields.issubset(dialogue.keys()):
            st.error(f"Dialogue {dialogue.get('dialogue_id', 'Unknown')} is missing required fields.")
            st.stop()
        # Validate 'turns' structure
        for turn in dialogue['turns']:
            if not {'turn_number', 'utterance', 'intent', 'assistant_response'}.issubset(turn.keys()):
                st.error(f"Turn {turn.get('turn_number', 'Unknown')} in dialogue {dialogue['dialogue_id']} is missing required fields.")
                st.stop()
...
tab1, tab2, tab3, tab4, tab5, tab6, tab7, tab8, tab9, tab10 = st.tabs([
    "Overview", "Emotions", "Categories", "Resolution", "Time Slots", "Regions", "Services", "Sentiment Analysis", "Advanced Analysis", "Dialogue Viewer"
])
