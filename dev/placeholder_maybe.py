...
class DialogueGenerator:
    ...
    def load_existing_hashes(self) -> set:
        ...
        if os.path.exists(self.hash_file):
            ...
            self.logger.warning(f"Hash file '{self.hash_file}' not found. Hashes need to be regenerated.")
        return set()
    ...
    def generate_dynamic_scenario(self, category: str, service: str) -> str:
        ...
        Adds placeholders to the generated scenario.
        """
        try:
            system_prompt = (
            ...
            # Add placeholders to the scenario
            doc = self.nlp(scenario)
            scenario_with_placeholders = scenario
            placeholders_mapping = {}

            for ent in doc.ents:
                if ent.label_ == 'PERSON':
                    scenario_with_placeholders = scenario_with_placeholders.replace(ent.text, '[NAME]')
                    placeholders_mapping['[NAME]'] = ent.text
                elif ent.label_ == 'GPE':
                    # Assuming GPE entities are destinations
                    # Here, we need to determine if it's 'from' or 'to' destination based on context
                    # For simplicity, replace all GPEs with [DESTINATION]
                    # You can enhance this by using dependency parsing or regex to find context
                    scenario_with_placeholders = scenario_with_placeholders.replace(ent.text, '[DESTINATION]')
                    # Append to existing list or create a new list
                    if '[FROM_DESTINATION]' not in placeholders_mapping:
                        placeholders_mapping['[FROM_DESTINATION]'] = ent.text
                    else:
                        placeholders_mapping['[TO_DESTINATION]'] = ent.text
                elif ent.label_ == 'MONEY':
                    # Replace currency amounts with [CURRENCY]
                    # This might include both the currency type and the amount
                    # We'll replace only the currency symbol or code
                    currency_pattern = re.compile(r'(\bUSD|\bEUR|\bGBP|\bJPY|\bCNY|\bINR|\b[0-9]+)')
                    scenario_with_placeholders = currency_pattern.sub('[CURRENCY]', scenario_with_placeholders)
                    placeholders_mapping['[CURRENCY]'] = ent.text

            # Replace phone numbers if any
            phone_pattern = re.compile(r'\+?\d[\d -]{8,}\d')
            phone_matches = phone_pattern.findall(scenario_with_placeholders)
            for phone in phone_matches:
                scenario_with_placeholders = scenario_with_placeholders.replace(phone, '[PHONE_NUMBER]')
                placeholders_mapping['[PHONE_NUMBER]'] = phone

            self.logger.info(f"Scenario with placeholders: {scenario_with_placeholders}")
            self.logger.debug(f"Placeholders mapping for scenario: {placeholders_mapping}")
            return scenario_with_placeholders, placeholders_mapping  # Return both scenario and mapping
    ...
    def process_generated_dialogue(self, generated_dialogue: str) -> Tuple[List[Dict], Dict]:
        """
        Processes the generated dialogue text into a list of turns and collects placeholder mappings.
        """
        generated_turns = []
        placeholders_mapping = {}  # Initialize mapping dictionary
        ...
        # Add placeholders to the generated turns and collect mappings
        generated_turns, turn_placeholders = self.add_placeholders(generated_turns)
        
        # Merge all placeholder mappings
        for placeholder, value in turn_placeholders.items():
            if placeholder in placeholders_mapping:
                # If placeholder already exists, append to list
                if isinstance(placeholders_mapping[placeholder], list):
                    placeholders_mapping[placeholder].append(value)
                else:
                    placeholders_mapping[placeholder] = [placeholders_mapping[placeholder], value]
            else:
                placeholders_mapping[placeholder] = value

        return generated_turns, placeholders_mapping

    def add_placeholders(self, turns: List[Dict]) -> Tuple[List[Dict], Dict]:
        """
        Replaces sensitive information in the dialogue with placeholders and collects mappings.
        Returns the modified turns and a mapping dictionary.
        """
        placeholders_mapping = {}  # Initialize mapping dictionary
        ...
        for turn in turns:
            ...
            # Contextual Replacement for Destinations
            # Using simple heuristics to detect 'from' and 'to' destinations
            # This can be enhanced with dependency parsing or more sophisticated methods
            from_destinations = []
            to_destinations = []

            # Detect 'from' and 'to' using regex
            from_pattern = re.compile(r'from\s+([A-Z][a-zA-Z\s]+)')
            to_pattern = re.compile(r'to\s+([A-Z][a-zA-Z\s]+)')

            from_matches = from_pattern.findall(utterance)
            to_matches = to_pattern.findall(utterance)

            # Replace 'from' destinations
            for match in from_matches:
                placeholder = '[FROM_DESTINATION]'
                utterance = utterance.replace(f"from {match}", placeholder)
                from_destinations.append(match)
                placeholders_mapping[placeholder] = match

            # Replace 'to' destinations
            for match in to_matches:
                placeholder = '[TO_DESTINATION]'
                utterance = utterance.replace(f"to {match}", placeholder)
                to_destinations.append(match)
                placeholders_mapping[placeholder] = match
            ...
            for ent in doc.ents:
                if ent.label_ == 'PERSON':
                    placeholder = '[NAME]'
                    utterance = utterance.replace(ent.text, placeholder)
                    placeholders_mapping[placeholder] = ent.text
                elif ent.label_ == 'GPE':
                    # If not already replaced by 'from' or 'to' destination, replace with [DESTINATION]
                    if not from_matches and not to_matches:
                        placeholder = '[DESTINATION]'
                        utterance = utterance.replace(ent.text, placeholder)
                        placeholders_mapping[placeholder] = ent.text
                elif ent.label_ == 'MONEY':
                    placeholder = '[CURRENCY]'
                    # Replace the currency code or symbol along with the amount
                    # Assuming format like '$75', 'USD 75', etc.
                    money_pattern = re.compile(r'(\bUSD|\$|EUR|£|JPY|CNY|INR)\s?\d+')
                    utterance = money_pattern.sub(placeholder, utterance)
                    placeholders_mapping[placeholder] = ent.text

            # Use regex to identify phone numbers and replace them with [PHONE_NUMBER]
            phone_pattern = re.compile(r'\+?\d[\d -]{8,}\d')
            phone_matches = phone_pattern.findall(utterance)
            for phone in phone_matches:
                placeholder = '[PHONE_NUMBER]'
                utterance = utterance.replace(phone, placeholder)
                placeholders_mapping[placeholder] = phone

            # Optionally, handle other entities like email addresses
            email_pattern = re.compile(r'\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}\b')
            email_matches = email_pattern.findall(utterance)
            for email in email_matches:
                placeholder = '[EMAIL_ADDRESS]'
                utterance = utterance.replace(email, placeholder)
                placeholders_mapping[placeholder] = email

            turn['utterance'] = utterance

        return turns, placeholders_mapping
    ...
    def generate_unique_dialogues(self, num_generations: int, min_turns: int, max_turns: int,
            ...
            for idx, dataset_index in enumerate(tqdm(selected_indices, desc="Generating dialogues")):
                ...
                generated_scenario, scenario_placeholders = self.generate_dynamic_scenario(selected_category, primary_service)
                ...
                if generated_dialogue:
                    generated_turns, dialogue_placeholders = self.process_generated_dialogue(generated_dialogue)
                    ...
                    # Merge scenario placeholders and dialogue placeholders
                    combined_placeholders = {**scenario_placeholders, **dialogue_placeholders}
                    ...
                    new_dialogues.append({
                        ...
                        'placeholders': combined_placeholders            # **Added placeholders mapping**
                    })
